...
- **Sprint ACTIVO:** **NETFLIX-TRIAL** (2/2 días, 2026-05-27 — branch `feature/netflix-trial-flow`). Stripe Checkout setup mode captura tarjeta upfront antes de que cliente entre a Zenix (Netflix/Spotify retention pattern). Sub queda `pending_payment_method` post-wizard; webhook `setup_intent.succeeded` crea Stripe Sub real con `default_payment_method` + `trial_period_days`. 856/890 backend tests verdes (10 nuevos Netflix; 9 fails pre-existentes en main, no relacionados). Sprint anterior cerrado: **BILLING-DISCOUNT-CODES** + **BILLING-CORE** (PR #45+#46 mergeados a `main` 2026-05-27) — Stripe subscriptions LIVE end-to-end + wizard E2E + receipt email template Anthropic-inspired + Salesforce CPQ pattern para descuentos consultor.
- **Pendientes post-NETFLIX-TRIAL:**
  - **CHANNEX-AUTO-PROVISION** (siguiente sprint, 9-11d): wizard crea property + room types + rate plans + OTAs en Channex automáticamente al activar. Plan v2 alineado con cert Stage 4 en [docs/sprints/CHANNEX-AUTO-PROVISION-plan.md](docs/sprints/CHANNEX-AUTO-PROVISION-plan.md).
  - **Fix 9 tests pre-existentes en main**: `properties.soft-delete.spec`, `availability.service.spec`, `access-control.service.spec` rompieron tras BILLING-CORE merge (TenantContextService methods refactor). No bloquea el branch Netflix — el commit que rompió fue mergeado antes que Netflix branchea de main.
