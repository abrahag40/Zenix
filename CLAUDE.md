...
- **Sprint ACTIVO:** **CLIENT-RETENTION-DISCOUNTS** o **WIZARD-CLOSE** (decisión owner). WIZARD-E2E (branch `feature/wizard-e2e` PR pendiente) cerrado 2026-05-29. PRs mergeados a main: #47 (5 sprints + plan) + #48 (BILLING-DAY1) + #49 (DISCOUNT-APPROVAL-UI) + #50 (PAC-CLIENT-WARNING) + #51 (CHANNEX-CERT-B1). Orden restante v1.0.0: CLIENT-RETENTION-DISCOUNTS (1-2d) → close wizard plan → CHECK-IN modal redesign → RATES-METRICS-COMPSET-CORE → QA-α → CI-RESCUE → Channex Stage 4 walkthrough → tag v1.0.0. Total ~25-36 días-dev = ~7-8 sem calendar. Target ago-sep 2026.
...
> **Sprint WIZARD-E2E — implementación cerrada 2026-05-29 con 4 tests reconocidos por Playwright list (happy + 3 edge cases). Typecheck web verde — la carpeta `e2e/` está fuera del `include` del tsconfig productivo (`include: ["src"]`), Playwright usa su propio loader. CI integration deferida a sprint separado. PR pendiente de creación.**
...
**Total restante:** ~32-43 días-dev secuencial 1 dev = ~7-9 semanas calendar. Target v1.0.0: **agosto-septiembre 2026**.
