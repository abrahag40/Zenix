...
## Estado actual del proyecto (2026-06-09)

- **Versión en curso:** v1.0.0 (piloto comercial — Hotel Monica Tulum). **SCOPE REABIERTO 2026-06-11 (Opción C) — tag DIFERIDO hasta cerrar AUTO-CHECKIN.**
- **Sprint ACTIVO: AUTO-CHECKIN** (branch `feat/auto-checkin`, arrancado 2026-06-11). Plan completo en [docs/sprints/AUTO-CHECKIN-plan.md](docs/sprints/AUTO-CHECKIN-plan.md) (APROBADO). Pre-arrival identity capture: al recibir un booking de Channex (email + tel), Zenix envía un email lindo con link a una mini web-app token-gated (`/precheckin/:token`) donde el huésped corrige sus datos (pre-cargados de Channex) + toma foto de pasaporte desde su móvil → se escribe en la reserva → agiliza (NO elimina) el check-in en recepción. Estimado ~2-3 sem (70% ensamblaje de piezas existentes). Decisiones bloqueadas con owner:
  - **El link NUNCA expone el ID interno** — token opaco; el ID solo viaja en JSON server-side. NO IDOR (D-AC1, patrón `setupTokenHash` §179).
  - **Pagos = SIEMPRE en recepción.** Auto-checkin NO procesa pago. **Carga del huésped = OPCIONAL** (best-effort; si no carga, check-in normal).
  - **Sin e-signature / T&C / NOM-151** (eso es SIGN-DLC v1.1.0; este MVP es su cimiento, el kiosk `/checkin-portal/:token`).
  - **Storage de la foto = reusar `UploadsService`** (Sharp + EXIF strip + UUID + 5MB, ya existe Mx-1B) con nuevo scope `precheckin`; disco ahora / R2 vía swap Mx-1C planeado. **Retrieval del pasaporte AUTH-GATED** (staff-only; NO por el GET público de uploads). Retención: purgar imagen ~30d post-checkout (configurable; tensión Visa 120d vs LFPDPPP). Detalle D-AC4.
  - **Trigger del email = temporizado pre-arrival** (reusa scheduler §41), NO inmediato al booking. Canal email (Resend) MVP; WhatsApp v1.0.x+.
  - **Precedencia write-back:** dato corregido por huésped gana sobre OTA modify (`guestVerifiedFields`, el `BookingModifyHandler` §136 no lo sobrescribe).
- **BOOKING-ENGINE diferido a v1.1.0** (Opción C): "interfaz propia de reservas" (Zenix Booking API + Booking Page hosted, `source='DIRECT_WEB'`, cero comisión OTA). Plan existente: [docs/sprints/BOOKING-ENGINE-plan.md](docs/sprints/BOOKING-ENGINE-plan.md) (5-6 sem). **Razón de no meterlo a v1.0.0:** depende de procesar el pago del huésped (Payment Intents Stripe guest→hotel + payouts + PCI) = territorio PAY-CORE (v1.0.1) → arrastraría PAY-CORE y duplicaría el release.
- **Tras AUTO-CHECKIN:** bump versiones `0.0.1`→`1.0.0` + `git tag v1.0.0` + walkthrough Channex Stage 4 (ops, paralelo).
- **📋 TABLA DE SEGUIMIENTO AUTO-CHECKIN** (se actualiza + reporta al cerrar cada fase; fuente de verdad anti-pérdida-de-sesión):

  | Fase | Detalle | Entregables clave | Estado |
  |------|---------|-------------------|--------|
  | **1a — Backend core** | Token opaco SHA256 (anti-IDOR) + endpoints públicos `GET/POST /v1/precheckin/:token` + write-back con `guestVerifiedFields` + foto vía `UploadsService` scope `precheckin` (orgId resuelto del token, sin TenantContext) | migración `20260616000000_auto_checkin_precheckin` (5 cols `precheckin*`/`guestVerifiedFields`); `PrecheckinService`/`Controller`/`Module`; UploadsService scope+orgId override; **11/11 tests** | ✅ **HECHO** (2026-06-11) |
  | **1b — Email + trigger** | `PrecheckinEmailService` (Resend, HTML emerald, fail-soft) + `PrecheckinScheduler` `@Cron('0 * * * *')` 2 pases idempotentes: invitación **3d antes** + recordatorio **24h antes** si no completó; precedencia: `BookingModifyHandler` (§136) NO pisa `guestVerifiedFields`. Marca solo en éxito o `no-key` (errores transitorios reintentan). TTL del token cubre hasta la llegada. | migración `20260617000000` (`precheckinReminderSentAt`); email+scheduler+modify guard; **16/16 tests** (6 scheduler + 2 email + 8 modify) | ✅ **HECHO** (2026-06-11) |
  | **2 — Web-app huésped** | Ruta pública `/precheckin/:token` en `apps/web` (form pre-llenado con datos de Channex + cámara móvil `capture="environment"` + compresión client-side ~1600px/JPEG0.8 + aviso de privacidad LFPDPPP + consentimiento + estados loading/ready/expired/notfound/submitted), mobile-first | `PrecheckinPage.tsx` + ruta pública en App.tsx; typecheck web verde | ✅ **código + typecheck** (render e2e en móvil → Fase 4) |
  | **3 — Integración recepción + storage** | (a) **Seguridad D-AC4**: scope `precheckin` bloqueado del GET público de uploads (404). (b) **Foto auth-gated**: `getCheckinContext` resuelve la foto de disco a data-URI server-side (solo staff autenticado la ve). (c) **Badge recepción**: `ConfirmCheckinDialog` muestra "Identidad pre-cargada por el huésped" + nº de campos verificados. (d) **Retención**: `PrecheckinRetentionScheduler` `@Cron('0 5 * * *')` purga la foto + anula `documentPhotoUrl` ~30d post-checkout (conserva `precheckinSubmittedAt`/`guestVerifiedFields` como rastro). | uploads guard + helpers `readAsDataUri`/`deleteByUrl`; context+badge; retention scheduler; **43/43 tests** (incl. regresión check-in) | ✅ **HECHO** (2026-06-11) |
  | **4 — QA e2e + navegador móvil** | **Verificación e2e en Chrome DESDE 0** (2026-06-11): stack arriba + seed booking tipo-Channex (Hans Müller/hostelworld) + token → `/precheckin/:token` en viewport móvil **renderiza con datos pre-cargados** (Hans/Müller/email/+49/DE, sin IDs internos) → submit (datos+foto) **write-back OK** (`guestPhone` corregido, `guestVerifiedFields`, `documentType`, foto) → **seguridad: GET público de la foto = 404** → **contexto auth-gated resuelve la foto a data-URI** → **recepción `ConfirmCheckin` muestra banner "Identidad pre-cargada por el huésped · 7 datos" + FOTO ✓ CAPTURADO**. | reporte QA e2e (este turno) | ✅ **HECHO** (1 salvedad: ver nota) |
  - **Salvedad honesta Fase 4:** el POST del **submit por la página** del huésped aterrizó server-side (datos en BD), pero la pestaña automatizada se congeló en CDP y NO se observó la pantalla de éxito "¡Listo!" del huésped — **verificar manualmente en un teléfono real** que la UI transiciona a "submitted". Además, la inyección de archivo en el `<input>` por automatización no dispara el `onChange` de React (limitación de herramienta, como el drag&drop) → la **foto** se validó por el endpoint real (curl) + se vio CAPTURADA en recepción. Todo lo demás verificado en navegador.
