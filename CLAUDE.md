...
| **CHANNEX-COMMAND-CENTER** | Multi-OTA control center en Zenix: refactor `/settings/channex` a multi-tab CRUD pleno (Room Types, Rate Plans, Rate Calendar matrix con parity alerts color-coded, Restrictions, Channels pause/unpause, Mappings wizard, Audit log). 3-tier RBAC consultor/supervisor/receptionist (SAP+SuccessFactors model). 10 decisiones D-CHX-CC-1..10 aprobadas 2026-05-23 PM. Diferenciador único: 5 capabilities que ningún PMS del estudio (Mews/Cloudbeds/Opera/LH/RR/Sirvoy) cubre simultáneamente — rate-parity alerts, RBAC granular, channel pause, rate caps Salesforce-style, mapping wizard health-check. Resuelve queja Capterra 5/5 PMS "I need 2 tabs open all day". Ver [docs/sprints/CHANNEX-COMMAND-CENTER-plan.md](docs/sprints/CHANNEX-COMMAND-CENTER-plan.md). | 12-16 | Sí |
