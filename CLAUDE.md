...
> **Estado:** **Sprints 0-6 CERRADOS (2026-06-13/14)**. Sprint 7 pendiente. Plan SCRUM completo en **[docs/sprints/MIGRATION-CORE-plan.md](docs/sprints/MIGRATION-CORE-plan.md)**. Módulo comercial 9 en [zenix-sales-master.md](docs/zenix-sales-master.md). v1.1.x DLC/servicio — **NO bloquea v1.0.0**. **Sprints 4-6 NO mergeados a main** (regla owner 2026-06-14: **solo desarrollar + commit en rama, sin PR ni CI hasta que el owner lo pida** — mergear a main dispara auto-deploy Render+Vercel). **Importación masiva FUNCIONALMENTE COMPLETA** — 3 caminos (plantilla Zenix / adapter dedicado Cloudbeds·Mews / CSV genérico con wizard de mapeo) + preview/resolución/gate + load idempotente + reporte + runbook de servicio asistido. **Próximo: Sprint 7 (QA con export real del piloto — corre cuando exista el archivo).**
>
...
>
> **✅ Sprint 6 entregado (rama `feat/migration-sprint6`, NO mergeado):** **Wizard de mapeo + runbook + spike Mews.** **US-6.1 wizard de mapeo** (camino flexible para exports sin plantilla): `GET /v1/nova/migration/fields` (campos canónicos = `TEMPLATE_FIELDS`); un job `GENERIC_CSV` queda `PARSING` sin `columnMapping` hasta que el consultor mapea → `POST …/mapping` → valida → `PREVIEW_READY`. Frontend: paso "Mapea las columnas" con **auto-sugerencia** (campo↔encabezado normalizado), select por campo, formato de fecha, preview de filas crudas, gate mínimo (sourceId/checkIn/checkOut). **US-6.2 runbook** [migration-assisted-runbook.md](docs/ops/migration-assisted-runbook.md) (cutover parallel-run→freeze→delta→reconectar OTAs vía Channex→sign-off). **US-6.3 `MewsAdapter`** pre-mapeo del Reservation Report (ASSUMED) — genericidad del motor probada e2e con 2º PMS sin tocarlo. **Hallazgo+fix:** el `csv-parser` solo manejaba `,`; export con `;` (Excel es-MX/es-ES, coma=decimal) caía en 1 columna → agregado `detectDelimiter()` (auto-detecta `,`/`;`/tab fuera de comillas), descubierto en el e2e real. **Tests 49/49** (Mews + 4 delimitador) + typecheck api/web verdes. **e2e BD dev** (Cancún): (A) Mews pre-mapeo → upload PREVIEW_READY directo; (B) genérico `;` → PARSING (8 headers OK) → fields 22 → applyMapping → PREVIEW_READY 0 conflictos → LOAD COMPLETED 1 cargada (`MX-M-2611-0001`); 117→117. **Salvedad (§4):** wizard UI no capturado en navegador (Vite del dueño en :5173); binding directo a endpoints verificados. **Importación masiva FUNCIONALMENTE COMPLETA.** **Próximo: Sprint 7** — QA con export real del piloto.
