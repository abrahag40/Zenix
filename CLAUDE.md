...
> **Última actualización:** 2026-05-21 (Sprint LEARNING-CORE planificado — Zenix Learning LMS como Add-On/DLC del PMS. Research completo en `docs/zenix-learning/` con 11 docs: legal LFT/STPS, competencia SuccessFactors+4, 3 cursos MVP (Distintivo H + NOM-035 / Front Office AHLEI / Housekeeping), psicopedagogía, arquitectura, UX, gamificación roadmap, mobile, pricing, plan implementación. Rama `claude/zenix-learning-lms-v1-2qz8l`. Target merge: v1.0.0 ideal / v1.1.0 fallback. Decisiones §128-§137 reservadas al cerrar sprint.)
...
| **[docs/zenix-learning/](../../docs/zenix-learning/)** | **Zenix Learning LMS — Add-On/DLC: 11 docs (legal LFT/STPS, competencia SF, 3 cursos MVP, psicopedagogía, arquitectura, UX, gamificación, mobile, pricing, plan)** |
...
- **Últimos commits relevantes:** PR #28-31 (bug-fixes UI + same-day turnover TZ-safe), **PR #32 mergeado** (Sprint CANCEL-ARCHIVE completo + 3-LEVEL Rates + FX-CORE Banxico + modal estándar + scroll perf SwiftUI)
- **Próximo bloque:** ~~SEC-α~~ ✅ → ~~POLISH-α~~ ✅ → ~~Mx-1B finalización~~ ✅ (PR #13) → ~~HK-CFG~~ ✅ (Sprint 8H) → ~~Bug-fixes UI + turnover~~ ✅ (PR #28-31) → ~~CANCEL-ARCHIVE + 3-LEVEL Rates + FX-CORE~~ ✅ (PR #32) → **CHANNEX-INBOUND** → **CHECK-IN modal redesign** → **QA-α** → **CI-RESCUE** → release v1.0.0
- **Bloqueantes nuevos identificados 2026-05-16:** Sin Channex inbound real, reserva OTA = invisible para recepción → overbooking + chargeback Visa 13.7. Plan documentado [CHANNEX-INBOUND-plan.md](docs/sprints/CHANNEX-INBOUND-plan.md) con propuesta + research citado.
...
| **LEARNING-CORE** (nuevo 2026-05-21) | Zenix Learning LMS Fase 1 (Add-On/DLC) — schema Prisma 14 modelos + módulo NestJS + web learner/manager + mobile (tab Hub) offline-first audio-first + 3 cursos MVP (Distintivo H + NOM-035 / Front Office AHLEI / Housekeeping) + DC-3 generator (LFT Art. 153-U) + gamificación ligera con roadmap a profunda + Activate wizard integration + curso-regalo como hook comercial. Solo nativo HTML5/audio/video/PDF Fase 1 (SCORM/xAPI a Fase 2). Rama independiente `claude/zenix-learning-lms-v1-2qz8l`. Decisiones §128-§137 reservadas. Research + plan completos en [docs/zenix-learning/](docs/zenix-learning/) (11 docs). | 42 días (~8.5 sem focused / ~6 sem con 2 devs) | Estratégico — hook comercial. Target v1.0.0 ideal / v1.1.0 fallback |
...
- **2026-05-21** — **Sprint LEARNING-CORE planificado — Zenix Learning LMS Fase 1 como Add-On/DLC.** Research + planning sesión completa con 4 agentes paralelos:
  - **Agent A — Legal LFT/STPS + LATAM equivalentes** — Verificable. LFT Art. 153-A a 153-X obliga capacitar; NOM-035-STPS-2018 multa 250-5,000 UMA (UMA 2026 $117.31 = MXN $29k-$586k por hallazgo, multiplicable por trabajador). DC-3 sigue siendo PDF individual; DC-4 físico descontinuado abril 2025 → SIRCE web. **No existe API STPS para DC-3 = oportunidad clara de producto.** Equivalentes Colombia (SENA + Ley 50/1990), CR (INA + Código Trabajo), Perú (SUNAFIL + Ley 29783), Centroamérica (INTECAP/INFOP/INSAFORP-INCAF/INADEH), Argentina (Ley 24.557 + LCT). Distintivo H = NMX-F-605-NORMEX-2018 + capacitación 10h obligatoria 80% personal operativo + 100% mandos. Tendencia 2025: STPS +34% inspecciones (43k vs 32k 2024).
  - **Agent B — SuccessFactors + 4 competidores + LMS hotelería.** SF: "back-end Frankenstein", admins se quejan en 80%+ de reviews, authoring externo Articulate $1.5-3k/año aparte. Comparativa TalentLMS / iSpring Learn / Docebo / Moodle Workplace. AHLEI Academy + eHotelier + Typsy + EHL Edge + Tovuti. UI/UX consolidado: catálogo grid + Continue/Due/Assigned/Recommended dashboard + sidebar colapsable player + manager "who's falling behind" (NO leaderboard público). Mobile vs desktop split 78% laptop / 45% smartphone (overlap). 40+ fuentes G2/Capterra/Reddit citadas.
  - **Agent C — Contenido 3 cursos hoteleros.** Distintivo H + NOM-035 (combo compliance MX, syllabus 8 módulos 24h, examen 60 ítems + caso práctico, vigencia 1 año Distintivo / 2 años NOM-035). Front Office Excellence alineado AHLEI CFDR (9 módulos 20h, simulación PMS, examen interno Zenix ≠ certificación oficial AHLEI). Housekeeping Standards alineado CHHE + Distintivo H (10 módulos 28h, examen + práctica timed). Cada uno con casos prácticos, errores comunes, bibliografía verificable. Reviewer experto requerido pre-publicación.
  - **Agent D — Psicopedagogía + gamificación.** Andragogía Knowles (6 principios), Bloom revisada Anderson&Krathwohl 2001 (verbos + 4 tipos conocimiento), Microlearning Kapp 3-7min + Spacing Thalheimer, SRS SM-2 Woźniak 200-300% mejor retención. Cognitive Load Sweller (ICL/ECL/GCL). SDT Deci&Ryan: crowding-out effect → leaderboards públicos prohibidos. Flow Csikszentmihalyi → quiz adaptativo ELO. Octalysis 8 drives (Drives 1-3 White Hat OK; 6-8 Black Hat prohibidos; streaks con loss aversion = no ético contexto laboral). Hamari 2014 meta-análisis: gamificación funciona contextual. Kirkpatrick L1-L4 + Phillips L5 ROI obligatorios. 50+ fuentes académicas.
  - **Decisiones del usuario:** rama independiente `claude/zenix-learning-lms-v1-2qz8l`, target v1.0.0 ideal con fallback v1.1.0, 3 cursos confirmados (Distintivo H+NOM-035 / Front Office / Housekeeping), solo nativo HTML5+audio Fase 1 (SCORM/xAPI Fase 2), gamificación ligera Fase 1 con roadmap a profunda documentado.
  - **11 docs creados en [docs/zenix-learning/](docs/zenix-learning/)**: 00-README, 01-vision (5 capas L0-L4 monetización + hook curso-regalo Cialdini), 02-legal-compliance (Agent A), 03-competitor-analysis (Agent B), 04-architecture-plan (14 modelos Prisma + endpoints REST + 7 integraciones + fuzzy search pg_trgm + IFiscalAdapter-paridad MxStpsAdapter Strategy), 05-ui-ux-patterns (wireframes Learner mobile+web / Manager / Admin / Activate / WCAG / estados error), 06-courses-catalog-mvp (Agent C), 07-psychopedagogy-foundation (Agent D), 08-gamification-roadmap (Fase 1 Ligera → 2 Media → 3 Profunda con métricas gate cuantitativas + reglas éticas + anti-patterns), 09-mobile-integration (offline-first + audio-first Spotify-style + push anti-fatigue + Detox tests), 10-implementation-plan (Sprint LEARNING-CORE 42 días en 5 sub-fases + dependencias + DoD + riesgos + §128-§137 reservadas), 11-pricing-bundling (SKUs $7-$12 USD/staff/mes + hook regalo ROI 10:1-22:1 + marketplace Fase 2). Estimado real: 8.5 semanas focused / ~6 sem con 2 devs paralelos. Bloqueante crítico: reviewer experto (consultor SECTUR Distintivo H + AHLEI partner) — empezar búsqueda DÍA 1 del sprint para no atorar publicación de cursos.
