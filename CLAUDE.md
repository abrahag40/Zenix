...
- **Versión en curso:** v1.0.0 (piloto comercial — Hotel Monica Tulum). **SCOPE REABIERTO 2026-06-11 (directiva owner) — tag DIFERIDO.**
- **⚠️ Scope v1.0.0 reabierto (2026-06-11):** el owner decidió incluir en v1.0.0 dos capacidades guest-facing que estaban planeadas post-1.0.0:
  - **BOOKING-ENGINE** — "interfaz propia de reservas" (Zenix Booking API + Booking Page hosted, `source='DIRECT_WEB'`, cero comisión OTA). Plan existente: [docs/sprints/BOOKING-ENGINE-plan.md](docs/sprints/BOOKING-ENGINE-plan.md) (5-6 sem Fase 1).
  - **AUTO-CHECKIN** — pre-arrival identity capture: al recibir un booking de Channex (email + tel del huésped), Zenix envía email con link a una mini web-app token-gated donde el huésped corrige sus datos (pre-cargados de Channex) + toma foto de pasaporte desde su móvil → se escribe en la reserva → agiliza el check-in en recepción. Es la base del kiosk `/checkin-portal/:token` ya planeado en SIGN-DLC (sin e-signature/NOM-151 todavía). Plan propuesta: [docs/sprints/AUTO-CHECKIN-plan.md](docs/sprints/AUTO-CHECKIN-plan.md).
  - **Secuenciación EN DEFINICIÓN** (ver análisis/propuesta del owner-session 2026-06-11). El PMS core + el tag estaban listos (validación abajo intacta) — el tag espera a cerrar el nuevo scope.
