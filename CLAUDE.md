...
- **📋 TABLA DE SEGUIMIENTO AUTO-CHECKIN** (se actualiza + reporta al cerrar cada fase; fuente de verdad anti-pérdida-de-sesión):
  ...
  | **2 — Web-app huésped** | Ruta pública `/precheckin/:token` en `apps/web` (form pre-llenado con datos de Channex + cámara móvil `capture` + compresión client-side + aviso de privacidad LFPDPPP + estados ready/expired/submitted/error), mobile-first | página React + hook | ⏳ siguiente |
