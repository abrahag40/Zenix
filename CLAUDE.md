...
- **📋 TABLA DE SEGUIMIENTO AUTO-CHECKIN** (se actualiza + reporta al cerrar cada fase; fuente de verdad anti-pérdida-de-sesión):
  ...
  | **1b — Email + trigger** | `PrecheckinEmailService` (Resend, HTML emerald-branded, fail-soft) + scheduler pre-arrival (reusa patrón §41) que genera token + envía X días antes + marca `precheckinSentAt` + recordatorio opcional; precedencia: `BookingModifyHandler` (§136) NO pisa `guestVerifiedFields` | email service + scheduler + handler tweak + tests | ⏳ siguiente |
  | **2 — Web-app huésped** | Ruta pública `/precheckin/:token` en `apps/web` (form pre-llenado con datos de Channex + cámara móvil `capture` + compresión client-side + aviso de privacidad LFPDPPP + estados ready/expired/submitted/error), mobile-first | página React + hook | ⏳ |
