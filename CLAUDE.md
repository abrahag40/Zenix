...
- **Versión en curso:** v1.0.0 (piloto comercial — Hotel Monica Tulum). **FASE DE CIERRE.**
- **Sprint ACTIVO:** **V1.0.0-RELEASE-FINALIZATION** (branch `release/v1.0.0`, arrancado 2026-06-09). Objetivo: cerrar los últimos pendientes y taggear v1.0.0. Checklist restante: **QA-α mobile** (coverage Hub Recamarista, 4-5d) → **CI-RESCUE residual** (8 stale assertions + reactivar test step blocking, 0.5-1d) → **CHANNEX-STAGE-4-WALKTHROUGH** (screenshare reviewer, 1d; ⚠️ Booking CRS write requiere habilitación de cuenta Channex — gestión owner, NO bloquea cert) → **bump version + tag v1.0.0**. Reports core ya entregado (ver abajo); REPORTS-CORE completo es v1.0.3 (post-tag).
...
- **Plan de trabajo Bloque 1 (release v1.0.0):** ~~CHANNEX-INBOUND~~ (✅ cerrado) → ~~CHANNEX-UX-E2-E3~~ (✅ cerrado) → ~~NOVA-CHANNEX-COMMAND-CENTER~~ (✅ cerrado 2026-05-25, wizard Zenix Activate end-to-end funcional con 160/160 tests) → **CHECK-IN modal redesign** (1-2d, siguiente) → **RATES-METRICS-COMPSET-CORE** (20-23d, revenue blocker, plan docs/sprints/RATES-METRICS-COMPSET-CORE-plan.md) → **QA-α mobile** (4-5d) → **CI-RESCUE residual** (0.5-1d). **Progress Bloque 1: ~85% completado**. Restante ~25-31 días-dev secuencial = ~5-6 semanas calendar. Target tag v1.0.0: jul-ago 2026.
...
| ⏳7 | **CHECKIN-MODAL-REDESIGN** — Opción C aprobada owner (3 sub-fases): C1 cerrado 2026-05-29 (bug fixes + walk-in ancho + nacionalidad/género). C2 pendiente (ReservationGroup §153-§154 backend + auto-detection + walk-in fast-path botón). C3 pendiente (GroupCheckinDialog 3-modos §156). §229-§234 D-CHECKIN-C1-1..6. | C1 cerrado / C2-C3 6-9d | UX recepcionista + LATAM hostal |
| 8 | **RATES-METRICS-COMPSET-CORE** — 3 capas (Rate Plans + Dashboard métricas + Compset MVP). Revenue blocker + Channex cert Tests 2-8. Ver [plan completo](docs/sprints/RATES-METRICS-COMPSET-CORE-plan.md) | 20-23d | Revenue + cert |
| 9 | **QA-α mobile** — Coverage tests Hub Recamarista | 4-5d | — |
| 10 | **CI-RESCUE residual** — 8 stale tests + 9 mocks pre-existentes en main | 0.5-1d | — |
| 11 | **CHANNEX-STAGE-4-WALKTHROUGH** — Screenshare con reviewer Channex usando `docs/ops/channex-cert-stage4-walkthrough.md` | 1d | — |
| 12 | **Tag v1.0.0** | — | — |

**Total restante:** ~25-36 días-dev secuencial 1 dev = ~7-8 semanas calendar. Target v1.0.0: **agosto-septiembre 2026**.
