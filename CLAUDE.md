...
- **Sprint ACTIVO:** **CHECK-IN modal redesign** (siguiente). CLIENT-RETENTION-DISCOUNTS (branch `feature/client-retention-discounts` PR pendiente) cerrado 2026-05-29. **Wizard Zenix Activate cerrado oficialmente 2026-05-29** + 29 decisiones §200-§228 consolidadas. PRs mergeados a main: #47-#53 (7 sprints). Orden restante v1.0.0: CHECK-IN modal redesign (1-2d) → RATES-METRICS-COMPSET-CORE → QA-α → CI-RESCUE → Channex Stage 4 walkthrough → tag v1.0.0. Total ~23-34 días-dev = ~6-7 sem calendar. Target ago-sep 2026.
...
> **Sprint CLIENT-RETENTION-DISCOUNTS — implementación cerrada 2026-05-29 con 95/95 tests billing verdes (no cambios al backend pre-existente — solo UI nueva + endpoint thin wrapper sobre `getSubscriptionForOrganization`). Typecheck API + web verdes. Diferenciador comercial: ningún PMS LATAM tiene UI consultor para aplicar discount retención win-back con preview live de ahorro + auto-revert Stripe + cap+approval integrado.**
