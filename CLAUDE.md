...
- **Sprint ACTIVO:** **CHECK-IN modal redesign C2** (ReservationGroup §153-§154 + walk-in fast-path) — siguiente. CHECK-IN C1 (branch `feature/checkin-c1-bugfixes-walkin` PR pendiente) cerrado 2026-05-29. PRs mergeados a main: #47-#54 (8 sprints). Decisiones acumuladas §200-§234. Orden restante v1.0.0: CHECK-IN C2 (2-3d) → CHECK-IN C3 GroupCheckinDialog (3-4d) → RATES-METRICS-COMPSET-CORE → QA-α → CI-RESCUE → Channex Stage 4 walkthrough → tag v1.0.0. Total ~28-39 días-dev = ~6-8 sem calendar. Target ago-sep 2026.
