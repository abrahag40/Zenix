...
- **📋 TABLA DE SEGUIMIENTO AUTO-CHECKIN** (se actualiza + reporta al cerrar cada fase; fuente de verdad anti-pérdida-de-sesión):
  ...
  | **4 — QA e2e + navegador móvil** | **Verificación end-to-end en Chrome iniciando el flujo DESDE 0** (levantar stack → seed booking Channex → email/scheduler → abrir `/precheckin/:token` en navegador móvil → form pre-llenado + foto + submit → ver "identidad pre-cargada" en el check-in de recepción → verificar foto auth-gated NO accesible públicamente). 3 superficies + bitácora. | reporte QA e2e | ⏳ **siguiente** |
