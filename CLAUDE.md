...
- **🚀 Sprint ACTIVO (próxima sesión): RESERVATION-EDIT-PRECHECKIN (v1.1.x).** Gap confirmado en código: el PMS edita reservas por acciones estructuradas (extend/move/early-checkout/split/cancel) pero **NO permite cambiar la fecha de LLEGADA ni mover libremente el rango de una reserva PRE-CHECK-IN** (hoy: cancelar+recrear). Plan SCRUM completo (épica → 4 sprints → HU con criterios + decisiones D-REP-1..4 a confirmar con owner) en **[docs/sprints/RESERVATION-EDIT-PRECHECKIN-plan.md](docs/sprints/RESERVATION-EDIT-PRECHECKIN-plan.md)**. ~5-8 días-dev. Rama sugerida `feat/reservation-edit-precheckin`.
