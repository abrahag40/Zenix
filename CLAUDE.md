...
>
> 🔴 **2026-09-20 · ZENIX DESCONGELADO por decisión del CEO.** Estado medido desde el código (1 205 pruebas en verde, 1 roja de sandbox), el piloto Azucar como primer hotel real, y las **cuatro decisiones previas al primer sprint** en [`docs/ops/2026-09-20-descongelacion.md`](docs/ops/2026-09-20-descongelacion.md). El resto de esta cabecera describe el estado del 2026-05-29.
...
- **📊 Sesión REPORTS-REVAMP + CASH-DRAWER — CERRADA y MERGEADA a main 2026-06-15 ([PR #127](https://github.com/abrahag40/Zenix/pull/127), merge `61509e4`, CI verde, auto-deploy prod v0.1.0).** Entregado bajo el **Estándar de Reportes** ([docs/standards/reporting-standard.md](docs/standards/reporting-standard.md) — reporte = tabla operable + export, no modal):
  - **Infra reusable:** `ReportTable` (primitiva web) + `common/report-export.ts` (`buildReportXlsx`/`buildReportCsv`, exceljs) + `api/download.ts` + biblioteca `/reports`.
  - **7 reportes LIVE:** Turnos · Movimientos · Resumen diario (caja) · No-shows · Estadías · **Métricas diarias** · **Saldos vencidos**. Filtros/orden/totales/paginación/export, per-divisa SUM-able (D-CASH3).
  - **P4(a) Reportes programados por email:** `ScheduledReport` + scheduler `@Cron` multi-tz + `ReportGenerationService` (seam reusable) + Resend con adjunto + CRUD + run-now + UI `/reports/scheduled`. *(Entrega real requiere `RESEND_API_KEY`.)*
  - **Catálogo COMPLETO definido** ([docs/standards/report-catalog.md](docs/standards/report-catalog.md)) — todos los reportes (área/usuario/columnas+justificación/estado/país); 7 LIVE + 7 "Próximamente" (bloqueados por PAY-CORE/RATES/CFDI-CORE, sin data falsa) + 1 dashboard. **Estudio i18n LATAM** ([docs/research/report-internationalization-latam.md](docs/research/report-internationalization-latam.md)): fiscalidad SIEMPRE por entidad legal/país; consolidación multi-país SOLO gerencial. **Runbook de activación** (flip bloqueado→LIVE) en [docs/sprints/REPORTS-REVAMP-plan.md §10](docs/sprints/REPORTS-REVAMP-plan.md). Decisiones a §-numerar (D-REPORT1..6 ya en el estándar).
- **✅ Sprint RESERVATION-EDIT-PRECHECKIN — CERRADO en rama `feat/reservation-edit-precheckin` (2026-06-16, SIN merge hasta autorización owner).** Resuelve el gap confirmado: el PMS editaba reservas por acciones estructuradas (extend/move/early-checkout/split/cancel) pero NO permitía **reprogramar el rango `[checkinAt, scheduledCheckout)` de una reserva PRE-CHECK-IN** (adelantar/retrasar/alargar/acortar) — antes se hacía cancelando+recreando. Entregado en 4 sprints (commits `30a912f` S1 backend núcleo · `686bf10` S2 preview+recotizar · `0367145` S3 UI + 2 bug-fixes). Endpoint `POST /v1/guest-stays/:id/edit-dates` + preview `GET …/edit-dates-preview` + `EditReservationDatesDialog` + botón en `BookingDetailSheet`. Decisiones **D-REP-1..6** en sección dedicada abajo. Verificado e2e en navegador (5 flujos: directa/conflicto→alternativa/OTA/recotizar/guardas) + 222/222 tests guest-stays+availability + typecheck api+web verde. Plan: [docs/sprints/RESERVATION-EDIT-PRECHECKIN-plan.md](docs/sprints/RESERVATION-EDIT-PRECHECKIN-plan.md). **Pendiente: PR + merge tras validación owner.**
...
> **Estado:** **Sprints 0-5 CERRADOS (2026-06-13/14)**. Sprints 6-7 pendientes. Plan SCRUM completo en **[docs/sprints/MIGRATION-CORE-plan.md](docs/sprints/MIGRATION-CORE-plan.md)**. Módulo comercial 9 en [zenix-sales-master.md](docs/zenix-sales-master.md). v1.1.x DLC/servicio — **NO bloquea v1.0.0**. **Sprints 4-5 NO mergeados a main** (deploy a prod = al final del desarrollo, instrucción owner 2026-06-14 — mergear a main dispara auto-deploy Render+Vercel). **Importación masiva: motor completo (0-4) + plantilla SuccessFactors (5); queda funcionalmente completa tras S6 (wizard de mapeo flexible + runbook + spike Mews); S7 = QA con export real del piloto.** **Próximo: Sprint 6.**
>
...
>
> **✅ Sprint 4 entregado (rama `feat/migration-sprint4`, NO mergeado a main):** **Load idempotente + AuditLog + reporte.** `MigrationService.load(jobId, orgId, actor)` crea `GuestStay`+`StayJourney`+`StaySegment(ORIGINAL)` reales por fila aprobada con `source='MIGRATED'` + `migrationSourceId`/`migrationJobId` + `bookingRef` SRC `'M'` (`[CC]-M-[propCode]-[YYMM]-[SEQ]`, helper propio con advisory lock del contador). Estados terminales mapeados (`actualCheckout`/`noShowAt`/`cancelledAt`). **Gate:** `blocking>0` → 400. **Idempotencia:** P2002 sobre UNIQUE `(migrationJobId, migrationSourceId)` → cuenta `existing`, re-load no duplica; job COMPLETED → devuelve resumen. **Anti-overbook §35:** re-check `AvailabilityService` dentro de la tx bajo `pg_advisory_xact_lock('migrate:'+propertyId)`, salvo históricas (CHECKED_OUT) o empalmes ACEPTADOS. **Tolerancia por fila:** una fila falla → reporta, job termina COMPLETED/PARTIAL. **AuditLog `DATA_MIGRATED`** append-only `retentionPolicy=PERMANENT`. **Reporte:** `GET …/jobs/:id/report` HTML imprimible (sin Puppeteer §183) servido vía cliente HTTP central con `responseType:'text'` (§122, sin raw fetch) → blob. **Frontend:** botón "Importar a producción" → `load`; bloque resultado COMPLETED/PARTIAL con StatTiles + "Ver reporte". SSE de calendario NO disparado (migración histórica/masiva — evitar spam de eventos operativos; follow-up). **Tests 41/41** (4 nuevos del gate: blocking>0 / estado inválido / idempotente COMPLETED / IDOR) + typecheck api/web verdes. **e2e BD dev** (Hotel Cancún): gate 2→0 → LOAD 201 (2 cargadas `MX-M-…` + journey + segmento ORIGINAL/ACTIVE; 12 fallidas razón "sin habitación destino" — sample 101-107 no existe en Cancún 201-208, caso REASSIGN; 1 omitida) → re-load idempotente 0 nuevas → reporte 200 con refs `M-`; dato de prueba limpiado. **Salvedad (§4):** load verificado e2e vía HTTP+JWT (BD real), NO en navegador (Vite del dueño en :5173); test unit del load real (creación en tx) diferido a integración (gate sí cubierto). **Próximo: Sprint 5** — runbook de servicio asistido white-glove + spike `MewsAdapter` (probar genericidad del motor).
>
> **✅ Sprint 5 entregado (rama `feat/migration-sprint5`, NO mergeado — reorientado a petición del owner):** **Plantilla Zenix (patrón SuccessFactors).** El owner pidió el flujo de carga masiva tipo SuccessFactors (plantilla CSV que el consultor rellena exportando del PMS del cliente). Se subió de prioridad sobre el runbook/spike Mews (→ Sprint 6) por ser el onramp real. **CSV elegido sobre XLSX** para la ingesta (cero dependencia, ya parseamos CSV, universal, patrón SuccessFactors/SAP/Salesforce; XLSX-con-dropdowns = mejora UX v2). **Backend:** `ZenixTemplateAdapter` (`MigrationSource.ZENIX_TEMPLATE`, primero en el registry) con **mapeo identidad** (encabezado === campo canónico) + `dateFormat='YYYY-MM-DD'`; `TEMPLATE_FIELDS` (22 cols, fuente única) + `buildZenixTemplateCsv()`; endpoint `GET /v1/nova/migration/template` → `text/csv` attachment (Nova-guarded, sin acting-org). **Frontend:** banner "¿Usas la plantilla Zenix?" + botón "Descargar plantilla" (blob) + source default `ZENIX_TEMPLATE` + `migrationApi.template()` (`responseType:'text'` del cliente central, §122). **Doc** [zenix-import-template.md](docs/sprints/migration/zenix-import-template.md) (diccionario de datos). **Clave:** la plantilla simplifica la ENTRADA, no la VALIDACIÓN (empalmes/dupes/fechas igual ocurren a mano → detector+gate siguen aplicando). **Tests 44/44** (3 nuevos del adapter) + typecheck shared/api/web verdes. **e2e BD dev** (Cancún): sources[0]=ZENIX_TEMPLATE ✓; descarga 200 text/csv 22 cols ✓; llenar con cuartos reales 201/202 → upload `ZENIX_TEMPLATE` → PREVIEW_READY 0 conflictos (identidad emparejó) → LOAD COMPLETED 2 cargadas (`MX-M-…`) ✓; BD limpiada (117 stays). **Incidente (§4):** la API consume `@zenix/shared` desde **dist compilado** → el enum nuevo no estaba en dist → `MigrationSource.ZENIX_TEMPLATE=undefined` → upload 400 + un script de cleanup con `jobId=undefined` intentó borrar TODO (lo frenó el FK `segment_nights`, **cero pérdida**, verificado 117→117). **Lección: tras tocar enums/tipos de `packages/shared`, correr `npm run build` en shared antes del runtime e2e** (jest/tsc usan source vía paths, pero el runtime de la API usa dist). **Próximo: Sprint 6** — wizard de mapeo de columnas (exports sin plantilla, `GenericCsvAdapter`) + runbook servicio asistido + spike `MewsAdapter`. Importación masiva queda funcionalmente completa tras S6.
