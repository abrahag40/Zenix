...
- **Sprint ACTIVO:** **BILLING-DISCOUNT-CODES** (2026-05-27, ~4-5d). Plan en [docs/sprints/BILLING-DISCOUNT-CODES-plan.md](docs/sprints/BILLING-DISCOUNT-CODES-plan.md). Sale de `feature/billing-core` (BILLING-CORE Days 1-8 cerrados 2026-05-27 — Stripe subscriptions LIVE end-to-end + wizard E2E + receipt email template Anthropic-inspired). Resuelve riesgo UX: slider de descuento expone cap del partner tier durante setup con cliente presente — pattern Salesforce CPQ + Stripe Promotion Codes elimina ese riesgo.
- **Pendientes post-DISCOUNT-CODES:**
  - **Netflix-style trial flow** (owner pidió 2026-05-27): capturar payment method upfront durante setup link (no esperar a Customer Portal post-trial). Pattern Stripe `mode: 'setup'` Checkout o Stripe Elements inline en SetupPage. Razón retención — Netflix/Spotify pattern, "ya tenemos sus datos bancarios" reduce churn por olvido. Cambio quirúrgico: remover `allowIncompleteWithoutPaymentMethod: true` de `SubscriptionService.createSubscription` + nuevo step en SetupPage. ~1-2 días-dev.
  - **CHANNEX-AUTO-PROVISION** sprint nuevo (gap detectado owner 2026-05-27): wizard crea property + room types + rate plans en Channex API automáticamente al activar. Plan en [docs/sprints/CHANNEX-AUTO-PROVISION-plan.md](docs/sprints/CHANNEX-AUTO-PROVISION-plan.md). ~5-7 días-dev.
