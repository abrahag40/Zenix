...
function makeBillingEmailMock() {
  return {
    sendReceipt: jest.fn().mockResolvedValue({ sent: true, resendMessageId: 'rsd_test' }),
  } as never
}

function makeSubscriptionMock() {
  return {
    activateAfterSetupIntent: jest
      .fn()
      .mockResolvedValue({ activated: false, reason: 'no_pending_subscription' }),
  } as never
}
...
describe('WebhookHandlerService', () => {
  describe('idempotency', () => {
    it('skip si stripeEventId ya existe en DB', async () => {
      ...
      const service = new WebhookHandlerService(prisma, makeBillingEmailMock(), makeSubscriptionMock(), makePaymentsMock())
