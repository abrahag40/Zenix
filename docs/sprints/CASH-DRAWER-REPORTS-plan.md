...
> - **S5b (reportes admin + settings) — ✅ CERRADO 2026-06-14:** tab "Caja" en `/reports` (`CashReportSection`: resumen del día por divisa×método + por cajero con filtros Todos/Sobrantes/Faltantes + CSV; tabla de turnos con estado/fondo/diferencia; `ShiftReportDialog` con el reporte individual + arqueo over/short + Imprimir + CSV) + acciones del supervisor (`SpotCountDialog` arqueo sorpresa sin cerrar el turno · `ReconcileDialog` conciliar CLOSED con razón) + tab "Caja" en Ajustes (`CashSettingsSection`: exigir-turno/blind/umbral/auto-close/modelo de banco vía `GET`/`PATCH /settings`; `cash*` whitelisteados en `UpdateSettingsDto`). **Verificado en navegador con DATOS reales** (preview→API propia 3001): el tab Caja lista turnos reales (OPEN/RECONCILED), el shift report muestra cajero "Carlos López" + arqueo Esperado/Contado/Diferencia + Imprimir/CSV, Ajustes→Caja renderiza y `PATCH→GET /settings` persiste (cashShiftRequired/threshold/bankModel). Typecheck api+web verdes. `vite.config` proxy ahora configurable por `VITE_API_PROXY` (default :3000, sin cambio de comportamiento). **Diferido:** email de cierre (R6) requiere config de destinatarios — sprint aparte; el print (`window.print`) ya cubre la salida inmediata.
