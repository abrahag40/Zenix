...
3. **R3 — Migración resto + dashboards** (EN CURSO, por lotes — verificado e2e por reporte):
   ...
   - ⏳ **Métricas diarias · Saldos vencidos** + mover KPIs/gráficas al dashboard + **retirar `/reports/classic`**. → ver **§10 Handoff** (pendiente específico).
...
### ⏳ PENDIENTE ESPECÍFICO (próxima sesión)

**P1 — Reporte "Métricas diarias" (ADR/RevPAR/ocupación) como tabla exportable.**
...
**P2 — Reporte "Saldos vencidos (overstayed)" como tabla exportable.**
