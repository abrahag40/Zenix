...
3. **R3 — Migración resto + dashboards** (EN CURSO, por lotes):
   - **R3-lote1 — ✅ Resumen diario de caja** migrado (`/reports/cash-summary`, `CashSummaryReportPage`: pivots por método/divisa + por cajero con `ReportTable` + export .xlsx/CSV, endpoint `GET /v1/cash-reports/cash-summary/export`). **Con esto el módulo de Caja queda 100% bajo el estándar** (Turnos + Movimientos + Resumen). Verificado: render en navegador + .xlsx válido por HTTP + typecheck.
   - **Pendiente R3** (cross-módulo, siguiente lote — cada uno: adaptar su endpoint al contrato `{rows,total,totals}`+export y montar `ReportTable`): **No-shows · Estadías · Métricas diarias · Saldos vencidos (overstayed)**. Luego: mover sus KPIs/gráficas al dashboard y **retirar `/reports/classic`**. *(Se hacen por reporte para mantener verificación e2e por cada uno — no en bloque.)*
