...
## 8. Bitácora
- **2026-06-15 (cont.)** — P4(a) reportes programados por email cerrado (`25cc6fa`). **Catálogo completo definido** ([report-catalog.md](../standards/report-catalog.md)) + **estudio de internacionalización LATAM** ([report-internationalization-latam.md](../research/report-internationalization-latam.md)) con investigación de campo de 2 agentes (fuentes oficiales). Biblioteca renderiza los 15 reportes (7 LIVE + 7 "Próximamente" + 1 dashboard) (`4e0394e`). Runbook de continuación agregado al §10. P4(b/c)/P5 quedan definidos+bloqueados hasta PAY-CORE/RATES/CFDI-CORE (decisión owner: dejar como está, documentado).
...
| Métricas diarias (P1) | `/reports/metrics` | `GET /v1/metrics/report[/export]` | `apps/web/src/reports/MetricsReportPage` |
| Saldos vencidos (P2) | `/reports/overstayed` | `GET /reports/overstayed-table[/export]` | `apps/web/src/reports/OverstayedReportPage` |
| Reportes programados (P4a) | `/reports/scheduled` | `… /v1/reports/scheduled` (CRUD + `/:id/run-now`) | `apps/web/src/reports/ScheduledReportsPage` |
...
- ✅ **P4(a) — Reportes programados por email** (`/reports/scheduled`, modelo `ScheduledReport` + scheduler multi-tz + `ReportGenerationService` seam + `ScheduledReportEmailService` Resend con adjunto + CRUD SUPERVISOR + run-now). v1 cubre métricas/no-shows/estadías/saldos. Verificado e2e (genera xlsx real; email stub sin `RESEND_API_KEY`). Commit `25cc6fa`. **Salvedad:** entrega real requiere `RESEND_API_KEY`; reportes de caja como fast-follow (sus builders usan tenant context de request).
- ✅ **CATÁLOGO COMPLETO + ESTUDIO i18n** (commit `4e0394e`):
  - **[docs/standards/report-catalog.md](../standards/report-catalog.md)** ← fuente de verdad de TODOS los reportes (área, usuario, columnas + justificación, estado de datos, país). **Consultar SIEMPRE aquí antes de construir un reporte nuevo.**
  - **[docs/research/report-internationalization-latam.md](../research/report-internationalization-latam.md)** ← estudio LATAM (reportes fiscales por país + dueño multi-país BR/MX). Hallazgo: fiscalidad SIEMPRE por entidad legal/país; consolidación multi-país SOLO gerencial.
  - Biblioteca renderiza el catálogo completo (15 reportes, 5 áreas): 7 LIVE + 7 "Próximamente · depende de X" (sin data falsa) + 1 dashboard.
- ⏳ **P4(b/c) / P5** — definidos en el catálogo como 🔴 BLOQUEADO; ver runbook abajo.
...
### 🔭 RUNBOOK DE CONTINUACIÓN (futuros desarrollos — leer antes de retomar reportería)

> Todo lo definido vive en **[docs/standards/report-catalog.md](../standards/report-catalog.md)** (qué reportes, columnas, usuario, estado, país) + **[docs/research/report-internationalization-latam.md](../research/report-internationalization-latam.md)** (cómo internacionalizar). Esta sección es el "qué hacer cuándo".

**A) Activar un reporte 🔴 BLOQUEADO (flip a LIVE) cuando aterrice su módulo:**
1. Confirmar que el dato existe (PAY-CORE → folios/saldos; RATES → `ratePlanId`/`commissionRate`/`marketSegment`; CFDI-CORE → timbrado/impuestos).
2. Agregar un `build<Reporte>ReportRows(propertyId, params)` en el servicio dueño del dato (patrón EXACTO `buildNoShowReportRows`/`buildMetricsReportRows`): retorna `{ rows, totals, currency, ... }`, per-divisa SUM-able (D-CASH3 — totales agregan solo la divisa filtrada).
3. Agregar endpoints `GET /<area>/<key>-table[/export]` (SUPERVISOR, D-REPORT6) usando `buildReportXlsx`/`buildReportCsv` de `common/report-export.ts`.
4. Frontend: `XxxReportPage` (molde `NoShowReportPage` / `MetricsReportPage`) + ruta en `App.tsx`.
5. En `ReportsLibraryPage.tsx`: cambiar la card de `status:'blocked'` → `status:'live'` + `to:'/reports/<key>'`.
6. (Opcional) registrar el reporte en `ReportGenerationService` (`apps/api/src/reports/report-generation.service.ts`) para que sea **programable por email** (P4a) — agregar el `case` en `generate()` + la key en `SUPPORTED` + el DTO/UI de scheduled.

**B) Pendientes concretos por módulo (cuando exista la data):**
- **P5 · Guest ledger / folios abiertos · Cuentas por cobrar (city ledger)** → depende de **PAY-CORE**. Columnas en el catálogo (área Contabilidad/Fiscal).
- **P5 · Producción por canal y segmento · Producción por tarifa** → depende de **RATES** (`ratePlanId`, `commissionRate`, `marketSegment` en el folio).
- **P4(c)/Fiscal · Facturación fiscal (CFDI/DIAN/NFS-e/FEL/RVIE/Libro IVA) · Impuestos por concepto** → depende de **CFDI-CORE**. **País-específico**: un reporte por `FiscalRegime`, producido por el `IFiscalAdapter` del país (§89). Brasil vía Sovos (§93). Ver i18n §3-§5.
- **Consolidado de grupo (multi-propiedad)** → gerencial, reporting currency con FX explícito, marcado NO-fiscal. Scope Org/Brand del JWT (§68). Ver i18n §4.
- **P4(b) · Custom column/filter builder** → evolución de `ReportTable` (ROI bajo con ~7 reportes; reconsiderar cuando el catálogo esté más lleno).
- **P4(c) · Integración GL (CONTPAQi/QuickBooks/Xero)** → EL NORTE; un mapeo por Entidad Legal (no consolidado cross-país). Depende de PAY-CORE.

**C) Fast-follows de lo ya construido (deuda menor honesta):**
- **Reportes de caja programables por email**: sus builders (`buildShiftReportRows`/`buildTransactionsRows`/`getCashSummary`) usan `tenant.getPropertyId()` del request → para schedulearlos, envolver la llamada en `cls.run()` (nestjs-cls) sembrando el contexto, o aceptar `propertyId` explícito en los builders. Luego registrarlos en `ReportGenerationService`.
- **Cash-summary** (pivot divisa×método×cajero) no está en el generador v1 (estructura no es rows planos) — adaptar al schedulear.
- **P4a entrega real de email** requiere `RESEND_API_KEY` en el entorno (hoy stub `no-key`). El cron `ScheduledReportsScheduler` corre `0,30 * * * *` multi-tz.
- `cashier-shift.api.ts` tiene `downloadCsv` inline — migrar al `download.ts` compartido.

### Cómo retomar rápido (verificación con datos)
1. `git checkout feat/cash-drawer-reports` (o `main` si ya se mergeó).
2. API en `:3001` (`PORT=3001 npx nest start` en `apps/api`) + preview web con `VITE_API_PROXY=http://localhost:3001` (config `web-preview` en `.claude/launch.json`, gitignored). Login `s@z.co`/`123456` (SUPERVISOR). Al terminar, matar `:3001`/`:5174` y dejar `:3000`/`:5173` del owner intactos.
3. Para un reporte nuevo: copiar el molde `MetricsReportPage` + `metrics.service.buildMetricsReportRows`; consultar columnas en `report-catalog.md`.
