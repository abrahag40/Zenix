...
- **2026-05-15 (PM late)** — **Tax Catalog nativo + LATAM coverage matrix.** Tras investigación profunda 32 estados MX + 9 países LATAM + fricción competitiva, se decide construir catálogo nativo curado por rol `TAX_CURATOR` interno Zenix (no Avalara/Vertex/Sovos en v1.0.x). 4 decisiones nuevas §91-§94 en CLAUDE.md. Sección J agregada en `14-payment-currency-tax-architecture.md`. Brasil EXCLUIDO v1.0.x (post v1.2 con Sovos). DSA Tulum marcado AMBIGUOUS — wizard solicita modalidad. Matriz MX completa con cambios 2026 (Yucatán 5→4.5 %, plataformas digitales diferenciadas en QR/CDMX/GRO/QRO/JAL). Wizard objetivo: 6-8 clicks vs ~30 Cloudbeds.
